...
import distilling.server.regression_typechecking.TracingResult.AccessPath
...
object Direction extends Enumeration {
  ...
  def getDirection(path: AccessPath): Direction = {
    val argAccessCount = path.count(p => p.isInstanceOf[ArgLabel])
    ...
    direction match {
      ...
      case Direction.PostMustBeSubType => {
        val baseType = RichType.OrType.buildOrType(
          baseKnown(path).map(obs => RichType.fromAndTypeString(obs.`type`)).toList)
        Some(
    ...
    direction match {
      case Direction.PreMustBeSubType => {
        val postType = RichType.OrType.buildOrType(
          postKnown(path).map(obs => RichType.fromAndTypeString(obs.`type`)).toList)
        Some(
          path -> RelationFailure(
...
object RichType extends Enumeration {
  ...
  Uint8ClampedArray, WeakMap, WeakSet, EventEmitter, Stream = Value
