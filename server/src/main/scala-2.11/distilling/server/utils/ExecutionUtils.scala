...
import distilling.server.utils.WorkaroundTaskSupport._
import distilling.server.utils.executors._
...
object ExecutionUtils {
  ...
  def rightExecutor(imageName: String): ProcExecutor =
    if (!isDockerEnv && Globals.useDockerExecutors) ContainerizedProcExecutor(imageName)
    else if (!isDockerEnv && Globals.usePathExecutors)
  ...
  def executeWithExecutor(cmd: String,
    ...
    executor.executeCommand(cmd.split(" "), timeout, logStdout, printFailure, env)
  }
