...
export class APIType {
    ...
    getRuntimeTypes(): Set<RuntimeType> {
        return this.runtimeTypes;
    }

    getProperties(): Map<Label, APIType> {
    ...
    hasType(type: RuntimeType): boolean {
        return this.runtimeTypes.has(type);
    }
    ...
    collectPathTypePairs (): [AccessPath, RuntimeType][] {
        return //DISABLED BECAUSE NOT UNDERSTOOD this.collectSubTypes_(new AccessPath(C.ABS_IDENTIFIER, undefined));
    }

    private collectSubTypes_ (subPath: AccessPath): [AccessPath, RuntimeType][] {
        let currentTypes = _.map<RuntimeType, [AccessPath, RuntimeType]> (Array.from(this.runtimeTypes.values()), rt => [subPath, rt]);

        let collectedSubTypes: [AccessPath, RuntimeType][] = _.flatMap(Array.from(this.properties.keys()), (prop) => {
...
export abstract class RegressionType extends Object {
    ...
    isSame(t2): boolean {
        return this.toIdString() === t2.toIdString();
    }
}
...
export class BottomType extends RegressionType {
    lub(t2: RegressionType): RegressionType {
        if (t2 instanceof BottomType)
            return this;
        return t2;
    }

    toJSON():any {
        return {"kind": "BottomType"};
    }

    public toString(): string {
        return this._toString(null);
    }
...
export class PrimitiveType extends RegressionType {
    ...
    lub(t2: RegressionType): RegressionType {
        if(t2 instanceof BottomType || t2 instanceof HasType)
            return this;
        ...
        if(t2 instanceof PrimitiveType && t2.typeName == this.typeName) return this;
...
export class ObjectType extends RegressionType {
    ...
    public _toString(visited: Set<RegressionType>): string {
        let props = Object.keys(this.properties).sort((a, b) => a.toString().localeCompare(b.toString()));
        let s = "Object(";
        for(let p of props) {
...
export class FunctionType extends RegressionType {
    ...
    constructor(
        public properties: IDictionary<RegressionType>,
...
export class UnionType extends RegressionType {
    ...
    private static lubOrAdd(types: RegressionType[], tt: RegressionType): RegressionType[] {
        ...
        if(changed || !haslubbed) {
            if(ns.length == 0)
                throw new Error("Unexpected empty union type");
            return ns;
        }

        // el1 \cup el2 = el1 or el1 \cup el2 = el2, so to remove duplicates
        return types;
    }
    ...
    toJSON():any {
        return {
            "kind": "UnionType",
            "types": this.types.map((v) => v.toJSON())
        }
    }
}
