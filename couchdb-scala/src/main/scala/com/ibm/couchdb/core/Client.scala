...
class Client(config: Config) {
  ...
  val baseHeadersWithAccept =
    baseHeaders.put(Header("Accept", "application/json; charset=utf-8"))

  val baseUri = Uri(
    scheme = CaseInsensitiveString(if (config.https) "https" else "http").some,
    authority =
      Uri.Authority(host = Uri.IPv4(address = config.host), port = config.port.some).some)

  def url(resource: String,
          params: Seq[(String, String)] = Seq.empty[(String, String)]): Uri = {
    baseUri.copy(path = resource).setQueryParams(params.map(x => (x._1, Seq(x._2))).toMap)
  }

  def req(request: Request, expectedStatus: Status): Task[Response] = {
    log.debug(s"Making a request $request")
    client.toHttpService.run(request) flatMap { mresponse =>
      val response = mresponse.asInstanceOf[Response]
      log.debug(s"Received response $response")
      if (response.status == expectedStatus) {
        Task.now(response)
      } else {
        log.warn(
          s"Unexpected response status ${response.status}, expected $expectedStatus")
        for {
  ...
  def getRaw(
    ...
    val request =
      Request(method = GET, uri = url(resource, params), headers = baseHeadersWithAccept)
  ...
  def getBinary(resource: String, expectedStatus: Status): Task[Array[Byte]] = {
    val request = Request(method = GET, uri = url(resource))
  ...
  private def put[T: R](resource: String,
    ...
    val headers =
      if (!contentType.isEmpty)
        baseHeadersWithAccept.put(Header("Content-Type", contentType))
      else baseHeadersWithAccept
    val request =
      Request(method = PUT, uri = url(resource), headers = headers, body = entity.body)
    reqAndRead[T](request, expectedStatus)
  }

  def put[B: W, T: R](resource: String, expectedStatus: Status, body: B): Task[T] = {
  ...
  def post[B: W, T: R](
    ...
    EntityEncoder[String].toEntity(write(body)) flatMap { entity =>
      val request = Request(
        ...
        headers = baseHeadersWithAccept.put(Header("Content-Type", "application/json")),
        body = entity.body)
      reqAndRead[T](request, expectedStatus)
    }
  }
  ...
  def delete[T: R](resource: String, expectedStatus: Status): Task[T] = {
    val request =
      Request(method = DELETE, uri = url(resource), headers = baseHeadersWithAccept)
    reqAndRead[T](request, expectedStatus)
  }
}
