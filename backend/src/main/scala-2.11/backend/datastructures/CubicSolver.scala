...
class CubicSolver[V, T](cycleElimination: Boolean = true) {
  ...
  private def detectPath(from: Node, to: Node): List[Node] = {
    ...
    def detectPathRec(current: Node): List[Node] = {
      if (current == to) {
        ...
        current.succ.map(varToNode(_)).toSet.filter(!visited.contains(_)).exists {
          n: Node =>
  ...
  /**
    ...
    */
  private def collapseCycle(cycle: List[Node]): Unit = {
    if (cycle.nonEmpty) {
  ...
  def addConditionalConstraint(t: T, x: V, y: V, z: V): Unit = {
    ...
    if (xn.tokenSol.contains(t)) {
      ...
      xn.conditionals.getOrElseUpdate(t, mutable.Set[(V, V)]()).add((y, z))
    }
  }
  ...
  def getSolution: Map[V, Set[T]] = {
    ...
    varToNode.keys
      .map(v => v -> getOrPutNode(v).tokenSol.map(i => intToToken(i)).toSet)
      .toMap
  }
}
